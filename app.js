...
function renderReportsCharts() {
  ...
  const catColors = ['#00f2fe', '#38bdf8', '#6366f1', '#8b5cf6', '#10b981', '#f59e0b', '#ec4899'];
  let catIndex = 0;

  catBox.innerHTML = Object.entries(categoryCounts).map(([catName, count]) => {
  ...
  const statusCounts = {
    ...
    "In Use": inventoryItems.filter(i => i.status === "In Use" || i.status === "Assigned").length,
    "Awaiting Repair": inventoryItems.filter(i => i.status === "Awaiting Repair" || i.status === "In Repair").length,
    "Not Working": inventoryItems.filter(i => i.status === "Not Working").length,
  };
  ...
  statusBox.innerHTML = Object.entries(statusCounts).map(([statusName, count]) => {
    ...
    return `
      <div class="bar-chart-row">
        <div class="bar-meta">
          <span style="font-weight:600; color:var(--text-primary);">${statusName}</span>
          <span style="color:var(--text-muted);">${count} items (${pct}%)</span>
        </div>
        <div class="bar-track">
          <div class="bar-fill" style="width: ${pct}%; background: ${color};"></div>
